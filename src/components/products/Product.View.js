...
function Product({ id, titulo, author, img, value }) {
    ...
    function addCart(body, token) {
        ...
        const promise = postCart({ bookId: body.id, name: body.name, author: body.author, imagem: body.img, value: body.value }, token);
    ...
    return (
        <ProductBook onClick={() => addCart({ id, titulo, author, img, value }, dados.token)} borda={borda} >
            <h2>{titulo}</h2>
            <h3>{author}</h3>
